...
Run ` npm run prep:simple-fade-effect` from the root of the repository

## Step 1

After scaffolding the plugin, run `npm run start` to build the provided JavaScript and CSS that creates the fade-in effect.
...
```php
add_filter( 'render_block_core/cover', 'enqueue_files_for_one_core_block' );
add_filter( 'render_block_core/image', 'enqueue_files_for_one_core_block' );
