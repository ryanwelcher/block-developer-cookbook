...
Run ` npm run prep:custom-build-process`` from the root of the repository
...
This is a common pattern. You use the spread operator to add the context of the entry and then add your new ones. This will work with any other property (you will see this later) however, the scripts package has a very cool feature built into it that can detect blocks and build them as needed. You can re-enable this feature by importing it to and then adding it to the entry
...
You may have also noticed that there is now an empty variation-style.js file in your build directory. This is because Webpack creates a javascript file for every entry point even if it’s a CSS file!

You can clean this up by using the RemoveEmptyScriptsPlugin Webpack plugin. This plugin does exactly what the name implies, it removes empty scripts. You will need to extend the plugins section of the Webpack config to add this.
